<template>
    <div class="project-maintenance-container">
        <!-- 页面标题和操作按钮 -->
        ...
        <div class="search-filter-section">
            <t-row :gutter="20" justify="space-between">
                <t-col :span="4">
                    <t-input v-model="searchKeyword" placeholder="搜索项目名称或编码" clearable @clear="handleClearSearch"
                        @keyup="handleSearch">
        ...
        <div class="table-section">
            <t-table stripe stripe-color="blue" :data="projectStore.projectList" :loading="projectStore.loading"
        ...
        <div class="pagination-container">
            ...
            <div class="pagination-section">
                <t-pagination :total="projectStore.pagination.total" :page-size="projectStore.pagination.pageSize"
                    :current="projectStore.pagination.page" :page-sizes="[10, 20, 50, 100]"
        ...
        <t-drawer @confirm="handleFormSubmit" @cancel="handleDrawerClose" :header="drawerTitle"
            :visible="projectStore.drawerVisible" size="500px" placement="right" @close="handleDrawerClose">
            <t-form :data="formData" :rules="formRules" ref="formRef" label-align="top">
                <t-form-item label="项目名称" name="project_name">
                    <t-input size="large" v-model="formData.project_name" placeholder="请输入项目名称" />
                </t-form-item>
                <t-form-item label="项目编码" name="project_code">
                    <t-input size="large" v-model="formData.project_code" placeholder="请输入项目编码" />
                </t-form-item>
                <t-form-item label="排序" name="sort">
                    <t-input size="large" v-model="formData.sort" placeholder="请输入排序" />
                </t-form-item>
                <t-form-item label="项目描述" name="remark">
                    <t-textarea v-model="formData.remark" placeholder="请输入项目描述"
        ...
        <t-dialog :visible="deleteDialogVisible" title="确认删除" :width="400" @close="handleDeleteDialogClose">
            ...
            <div class="delete-dialog-content">
                <p class="delete-warning">确认删除项目「{{ selectedProjectName }}」? 此操作不可撤销，删除后数据将无法恢复。</p>
            </div>
            <template #footer>
...
const formRules = {
    project_name: [
        ...
        { min: 1, max: 50, message: '项目名称长度应在1-50个字符之间', trigger: 'blur' }
    ],
    project_code: [
        { required: true, message: '请输入项目编码', trigger: ['blur', 'change'] },
        { min: 1, max: 30, message: '项目编码长度应在1-30个字符之间', trigger: 'blur' },
        {
            pattern: /^[a-zA-Z0-9_]+$/,
            message: '项目编码只能包含字母、数字和下划线',
            trigger: ['blur', 'change']
        }
    ],
    sort: [
...
const drawerTitle = computed(() => {
    return projectStore.currentAction === 'add' ? '添加项目' : '编辑项目';
});
...
const handleAdd = () => {
    ...
    nextTick(() => {
        ...
        projectStore.openAddDrawer();
    });
};

// 处理编辑
const handleEdit = (id) => {
    projectStore.openEditDrawer(id).then(() => {
        // 填充表单数据
        if (projectStore.projectDetail) {
            Object.keys(formData).forEach(key => {
                formData[key] = projectStore.projectDetail[key] || '';
            });
        }
    });
};
...
const confirmDelete = async () => {
    try {
        await projectStore.deleteExistingProject(selectedProjectId.value);
...
const handleFormSubmit = async () => {
    try {
        ...
        if (projectStore.currentAction === 'add') {
            // 添加操作
            await projectStore.addNewProject(formData);
        } else {
            // 编辑操作
            await projectStore.updateExistingProject(projectStore.currentProjectId, formData);
        }
...
// 搜索
const handleSearch = () => {
    projectStore.setQueryParams({
        project_name: searchKeyword.value,
        project_code: searchKeyword.value,
        page: 1
    });
...
const handleClearSearch = () => {
    searchKeyword.value = '';
    projectStore.resetQueryParams();
    projectStore.fetchProjectList();
};

// 状态筛选
const handleStatusFilter = () => {
    projectStore.filterByStatus(selectedStatus.value);
};
...
const handlePageChange = (page) => {
    projectStore.changePage(page.current);
    projectStore.changePageSize(page.pageSize);
};
