  ...
  <div class="tree-operation-container">
    ...
    <div class="tree-wrapper">
      ...
      <div class="tree-node level-1">
        <t-input v-model="valueSetCode" placeholder="请输入值集编码" class="code-input" :disabled="isDisabled" />
        <t-button type="primary" @click="queryData" icon="add" class="add-btn" :disabled="!valueSetCode.trim()">
          查询
        </t-button>
        <t-button type="primary" @click="addChildNode" icon="add" class="add-btn" :disabled="!valueSetCode.trim()">
          新增节点
        </t-button>
      </div>
      ...
      <div class="child-nodes">
        <div v-for="(node, index) in childNodes" :key="index" class="tree-node level-2">
          <!-- 节点操作区 -->
          <div class="node-actions">
            <t-icon name="drag" class="drag-icon" cursor="move" title="拖动排序" />
            <t-icon name="delete" class="delete-icon" cursor="pointer" title="删除节点" @click="removeChildNode(index)" />
          </div>

          <!-- 二级节点输入框组 -->
          <div class="input-group level-2-inputs">
            <t-input v-model="node.meaning" placeholder="请输入名称" class="node-input name-input" :disabled="isDisabled" />
            <t-input v-model="node.value" placeholder="请输入值" class="node-input value-input" :disabled="isDisabled" />
            <t-input v-model="node.orderSeq" placeholder="排序号" class="node-input sort-input" type="number"
              :disabled="isDisabled" />
          </div>
          ...
          <div class="level-3-nodes">
            ...
            <div class="language-nodes">
              <div v-for="(lang, langIndex) in node?.languages || []" :key="langIndex" class="language-node">
                <t-input :value="lang?.name" readonly class="lang-name" />
                <t-input :value="lang?.value" placeholder="请输入对应语言值" class="lang-value" :disabled="isDisabled" />
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- 空状态提示 -->
        <div v-if="childNodes.length === 0" class="empty-state">
    ...
    <div class="action-buttons">
      <t-button type="default" @click="resetForm">
        重置
      </t-button>
      <t-button type="primary" @click="submitForm" :disabled="!valueSetCode.trim() || childNodes.length === 0">
        提交
      </t-button>
    </div>
  </div>
...
const queryData = async () => {
  try {
    ...
    if (headerResponse?.empty) {
      ...
      // 值集头信息存在
      valueHeader.value = headerResponse?.content?.[0];
      // 再获取值集行信息
      const rowsResponse = await getLovRows(valueHeader.value.lovId);
      ...
      if (rowsResponse.empty) {
        ...
        // 处理行数据，填充到子节点列表
        childNodes.value = rowsResponse?.content;
        for (let i = 0; i < childNodes.value.length; i++) {
          const row = childNodes.value[i];
          const languagesResponse = await getLovRowsLanguages(row._token);
          childNodes.value[i].languages = languagesResponse instanceof Array ? languagesResponse : []; // 初始化多语言值为空
        }
        MessagePlugin.success('数据查询成功');
        // 循环便历行数据，获取多语言数据
        // childNodes.value = childNodes.value.content.map(async (node, index) => {
...
const addChildNode = () => {
  ...
  childNodes.value.push({
    meaning: '',
    value: '',
    orderSeq: childNodes.value.length + 1, // 默认排序号为节点索引+1
